...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        {
            decltype(auto) c = compiled[0];
            auto serialized = linker.serialize(*c);
            decltype(auto) m = linker.deserialize(serialized);
            l2.provideModule(std::move(m));
            ...
            std::ofstream(c->name + ".bc") << serialized;
        }
